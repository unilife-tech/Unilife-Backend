...
class Admin_Controller extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->verify_login();
		$this->verify_login_token();

		// store site config values
		$this->mUsefulLinks = $this->mConfig['useful_links'];
	}

	protected function verify_login_token()
	{
		$user_session = $this->session->all_userdata();
		@$user_id = $user_session['user_id'];
		$token=$this->session->userdata('token');
		$this->load->model('custom_model');
		$query = "SELECT id,token from admin_users WHERE id='$user_id' ";
		$datacheck = $this->custom_model->get_data_array($query);
		if(!empty($datacheck))
		{
			if($datacheck[0]['token']!=$token)
			{
