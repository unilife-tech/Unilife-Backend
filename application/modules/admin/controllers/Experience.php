...
class Experience extends Admin_Controller {
	...
	public function create($user_id = '')
	{
		...
		if ( !empty($post_data) )
		{
			...
			$response = $this->custom_model->my_insert($post_data,'user_experience');
			
			if ($response)
			{
				// success
				$this->system_message->set_success('Experience created successfully');
			}
			else
			{
				// failed
		...
		$this->mPageTitle = 'Create experience';
	...
	public function edit($cate_id)
	{
		...
		if ( !empty($post_data) )
		{
			$cate_data = $this->custom_model->my_where('user_experience','*',array('id' => $cate_id));
			...
			// {
				// proceed to create Category
				$response = $this->custom_model->my_update($post_data,array('id' => $cate_id),'user_experience');
				
				if ($response)
				{
					// success
					$this->system_message->set_success('Experience Edited successfully');
				}
				else
				{
					// failed
		...
		$this->mPageTitle = 'Edit experience';
