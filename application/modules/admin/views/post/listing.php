...
<a target="_blank" class="delete_all_post btn btn-info btn-md " role="button">Delete All posts</a>
...
<script type="text/javascript">
    ...
    $('#loading').show();     
      $.ajax({
           type: 'POST',
           url: "<?php echo base_url("admin/post/index"); ?>",
...
<script type="text/javascript">
  $(document).on('click',".delete_all_post",function(){
    var post_id = 'all';
    // alert(post_id);
    // return false;
    ...
    if(post_id != '')
    {
      swal({
            title: "",
            text: "Are you sure you want to delete all post !!",
            type:"warning",                                  
            showCancelButton: true,                  
            confirmButtonText: "OK",
            cancelButtonText: "CANCEL",
          ...
          {         
            if (inputValue===true) {                
              ...
              success: function(response){                               
               if(response)
               {
                swal("","All post deleted successfully. ",'success');
                setTimeout(function(){ location.reload(); }, 2000);
               }else {
                swal("","Some thing want worng!!","warning");
               }
