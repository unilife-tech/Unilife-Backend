...
<table class="table table-bordered table-striped table-hover dataTable js-exportable">
   ...
   <tbody id="table_body">
      ...
      <tr>
         ...
         <td><a target="_blank" style="width: 30px;" href = "users/post/<?php echo $row['report_user_id']?>"  style="width: 30px;" class="see_post" role="button">See Post</a> </td>
         ...
         <td>
            ...
            <a  style="width: 30px;" data-id = "<?php echo $row['report_user_id'] ?>,<?php echo $row['againt_complain']?>,<?php echo $row['id'] ?>,"  style="width: 30px;" class="delete_user btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button"><i class="material-icons">auto_delete</i></a>
...
<script type="text/javascript">
     function creatTable(tabledata) {
      ...
      $.each(tabledata, function( k, v ) {   
          ...
          trHTML+='<td><a target="_blank"  style="width: 30px;" href = "users/post/'+v.report_user_id+'"  style="width: 30px;" class="see_post" role="button">See post</a>   </td>';

           trHTML+='<td>'+v.created_at+'</td>'; 

       
          trHTML+='<td>  <a  style="width: 30px;" data-id = "'+v.id+'"  style="width: 30px;" class="delete_report btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button"><i class="material-icons">delete</i></a>            <a  style="width: 30px;" data-id = "'+v.report_user_id+','+v.againt_complain+','+v.id+',"  style="width: 30px;" class="delete_user btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button"><i class="material-icons">auto_delete</i></a></td>';     
...
<script type="text/javascript">
  $(document).on('click',".delete_user",function(){
    ...
    if(post_id != '')
    {
      swal({
            title: "",
            text: "Are you sure you want to delete this " + post_id[1] + " user account !!" ,
            type:"warning",                                  
          ...
          function(inputValue){         
            if (inputValue===true) {                
              $.ajax({
              type: 'POST',
              url: '<?php echo base_url('admin/report/delete_complain_user') ?>',
              data: {c_uid:c_uid,c_id:c_id},
              success: function(response){                               
               if(response)
               {
                swal(""," " + post_id[1] + " user account deleted successfully. ",'success');
